...
lines.each do |l|
  dim = l.split("x").map(&:to_i).sort
  item = (3 * dim[0] * dim [1]) + (2 * dim[1] * dim[2]) + (2 * dim[0] * dim[2])
  totalArea += item
  ribbon = (2 * (dim[0] + dim[1])) + (dim[0] * dim[1] * dim[2])
  totalRibbon += ribbon
end

puts "The elves need to order #{totalArea} square feet of wrapping paper"
puts "and #{totalRibbon} feet of ribbon."
